...
@app.route('/covid19/api/v1/predict/', methods=['POST'])
def post_covid_classifier():
    img_path = request.json.get('img_path')
    filename = request.json.get('filename')
    logging.info(img_path)
    logging.info(filename)
    prediction, prob, img_pred_name = covid_classifier_model2(img_path, filename)
    response = {'prediction': prediction, 'prob': prob, 'img_pred_name': img_pred_name}
...
def covid_classifier_model2(img_path, filename):
    ...
    version = os.environ['COVID_VERSION'] 
    MODEL2_API_URL = 'http://123.41.0.179:30111/v1/models/covid19/versions/'+version+':predict'
...
def covid_classifier_model2_heatmap():
    ...
    #MODEL2_API_URL is tensorflow serving URL in another docker
    HEADERS = {'content-type': 'application/json'}
    MODEL2_API_URL = '/v1/models/covid19/versions/1:predict'
    CLASS_NAMES = ['Covid19', 'Normal_Lung', 'Pneumonia_Bacterial_Lung']
